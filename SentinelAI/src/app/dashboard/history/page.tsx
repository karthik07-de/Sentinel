...
import { History, Globe, Mail, Image, QrCode, Search, Filter } from "lucide-react";
import { useAppStore } from "@/store";
import { formatRelativeTime, cn } from "@/lib/utils";
import type { ScanType, SeverityLevel } from "@/types";
        ...
        <div className="divide-y divide-white/[0.05]">
          {filtered.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-slate-500 text-sm">No scans match your filters</p>
            </div>
          ) : (
            filtered.map((scan) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-slate-500 text-xs mt-0.5 capitalize">{scan.type} scan · {formatRelativeTime(scan.timestamp)}</p>
                  </div>
                  <div className={cn("px-2.5 py-1 rounded-lg text-xs font-semibold border flex-shrink-0", severityBadge[scan.severity])}>
                    {scan.riskScore}/100
                  </div>
                  <div className={cn(
